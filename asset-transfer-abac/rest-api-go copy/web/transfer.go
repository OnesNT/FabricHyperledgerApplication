...
func (setup *OrgSetup) Transfer(w http.ResponseWriter, r *http.Request) {
	...
	if chainCodeName == "" || channelID == "" || assetID == "" || recipientCN == "" {
	...
	fmt.Printf("channel: %s, chaincode: %s, function: TransferAsset, args: [%s, %s]\n", channelID, chainCodeName, assetID, recipient)
	...
	txn_proposal, err := contract.NewProposal("TransferAsset", client.WithArguments(assetID, recipient))
	if err != nil {
